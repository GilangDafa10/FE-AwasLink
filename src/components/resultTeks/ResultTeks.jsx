...
const ResultTeks = () => {
  ...
  // Ambil risk score
  const rawScore =
    scanResult?.data?.messageRiskScore ??
  ...
  const config = {
    danger: {
      ...
      description:
        "AI kami mendeteksi pola manipulatif kuat dan indikasi penipuan dalam teks pesan ini. Hindari berinteraksi lebih jauh.",
    ...
    warning: {
      ...
      description:
        "AI kami menemukan beberapa indikasi rekayasa sosial atau tautan yang mencurigakan. Harap berhati-hati sebelum menindaklanjuti.",
    ...
    safe: {
      ...
      description:
        "AI kami tidak menemukan adanya pola penipuan, manipulasi psikologis, atau link berbahaya pada teks ini.",
  ...
  // Detail analisis risiko berdasarkan status
  const riskDetailsConfig = {
    danger: [
      {
        icon: <Clock className="w-5 h-5" />,
        title: "Urgensi Buatan",
        description:
          "Pesan memaksa Anda bertindak cepat dengan ancaman waktu agar Anda tidak sempat berpikir jernih.",
      },
      {
        icon: <Lock className="w-5 h-5" />,
        title: "Permintaan Data Sensitif",
        description:
          "Terdapat ajakan untuk melakukan tindakan/verifikasi yang biasanya merupakan cara untuk mencuri kredensial.",
      },
      {
        icon: <MessageSquare className="w-5 h-5" />,
        title: "Bahasa yang Manipulatif",
        description:
          "Penggunaan kata-kata formal yang dipadukan dengan ancaman adalah taktik rekayasa sosial klasik.",
      },
      {
        icon: <Link2Off className="w-5 h-5" />,
        title: "Link Pendek (Shortened URL)",
        description:
          "Penggunaan link pendek menyembunyikan tujuan asli situs untuk mengelabui filter keamanan.",
      },
    ],
    warning: [
      {
        icon: <AlertTriangle className="w-5 h-5" />,
        title: "Indikasi Mencurigakan",
        description:
          "Pesan memiliki beberapa ciri yang dapat menunjukkan kemungkinan phishing atau penipuan.",
      },
      {
        icon: <Lock className="w-5 h-5" />,
        title: "Verifikasi Diperlukan",
        description:
          "Terdapat permintaan informasi atau aksi yang memerlukan verifikasi dengan saluran resmi.",
      },
      {
        icon: <MessageSquare className="w-5 h-5" />,
        title: "Pola Tidak Biasa",
        description:
          "Gaya bahasa atau format pesan berbeda dari komunikasi resmi yang umumnya Anda terima.",
      },
      {
        icon: <Link2Off className="w-5 h-5" />,
        title: "Tautan Perlu Diperiksa",
        description:
          "Link dalam pesan perlu dikonfirmasi sebelum diklik untuk memastikan keamanan.",
      },
    ],
    safe: [
      {
        icon: <ShieldCheck className="w-5 h-5" />,
        title: "Struktur Pesan Normal",
        description:
          "Pesan memiliki struktur dan format yang konsisten dengan komunikasi resmi yang aman.",
      },
      {
        icon: <MessageSquare className="w-5 h-5" />,
        title: "Bahasa Tidak Manipulatif",
        description:
          "Bahasa yang digunakan profesional tanpa tekanan psikologis atau ancaman yang mencurigakan.",
      },
      {
        icon: <Clock className="w-5 h-5" />,
        title: "Tidak Ada Urgensi Palsu",
        description:
          "Pesan tidak menggunakan taktik urgensi buatan untuk memaksa Anda bertindak cepat.",
      },
      {
        icon: <Lock className="w-5 h-5" />,
        title: "Permintaan Wajar",
        description:
          "Tidak ada permintaan data sensitif atau verifikasi mencurigakan yang tidak perlu.",
      },
    ],
  };

  const apiDetails = scanResult?.details || scanResult?.indicators;
  const defaultDetails = riskDetailsConfig[status];
  const displayDetails =
    Array.isArray(apiDetails) && apiDetails.length > 0
  ...
  const stepsConfig = {
    danger: {
      ...
      buttonClass:
        "bg-[#d32f2f] hover:bg-red-700 shadow-red-600/20 hover:shadow-red-600/40",
      action: () => window.open("https://aduannomor.id", "_blank"),
    },
    warning: {
      steps: [
        {
  ...
  return (
    <div className="max-w-7xl mx-auto py-8 md:py-10 px-6 md:px-12 bg-gray-50 min-h-screen font-sans">
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 flex flex-col gap-6">
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 md:p-8 flex flex-col md:flex-row gap-6 md:gap-8 relative overflow-hidden">
            <div
              className={`absolute left-0 top-0 bottom-0 w-2 ${config.barColor}`}
            ></div>

            {/* Warning Icon and Badge */}
            <div className="flex flex-col items-center shrink-0">
              <div
                className={`w-20 h-20 ${config.accentBg} ${config.accentText} rounded-full flex items-center justify-center mb-3 shadow-inner`}
              >
                {config.icon}
              </div>
              <div
                className={`${config.riskBadgeBg} text-xs font-bold px-3 py-1.5 rounded-full shadow-md`}
              >
                {riskPercent}% RISK
              </div>
            </div>
            ...
            <div className="flex-1">
              <div
                className={`inline-block ${config.badgeClass} text-xs font-bold px-3 py-1.5 rounded-md mb-4 uppercase tracking-wider border`}
              >
                {config.badgeText}
              </div>
              ...
              <div className="bg-[#f8f9fc] border border-blue-100/50 rounded-xl p-6 relative">
                <div
                  className={`absolute left-0 top-6 bottom-6 w-1 ${config.lineIndicatorColor} rounded-r-md`}
                ></div>
                <h3 className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-3">
                  Teks Pesan Yang Dianalisis
                </h3>
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 md:p-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              {displayDetails.map((detail, index) => (
                <div
                  key={index}
                  className="bg-[#f8f9fc] border border-blue-100/60 p-5 rounded-xl hover:shadow-md transition-shadow"
                >
                  <div
                    className={`flex items-center gap-3 mb-3 ${config.detailIconColor} font-semibold`}
                  >
                    {detail.icon}
        ...
        <div className="lg:col-span-1 flex flex-col gap-6">
            ...
            <ul className="space-y-6 flex-1 mb-8">
              {stepsConfig.steps.map((step) => (
                <li key={step.num} className="flex gap-4">
                  <div
                    className={`w-7 h-7 ${status === "safe" ? "bg-green-600/90 shadow-green-600/20" : "bg-red-600/90 shadow-red-600/20"} rounded-full flex items-center justify-center text-sm font-bold shrink-0 mt-0.5 shadow-lg`}
                  >
                    {step.num}
                  </div>
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden flex flex-col group cursor-pointer hover:shadow-md transition-shadow">
            ...
            <div className="p-6">
              ...
              <p className="text-sm text-gray-600 leading-relaxed">
                Deteksi bahasa berbasis NLP kami mengenali pola penipuan dalam
                berbagai bahasa secara instan.
              </p>
            </div>
          </div>
        </div>
